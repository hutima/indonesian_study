...
test('service worker caches the shell and every registered content unit for offline reads', async () => {
  ...
  for (const path of ['./index.html', './app.js', './progress.js', './vocab-deck.js', './vocab-charts.js', './vocab-sections.js', './navigation.js', './morphology-order.js', './content/morphology-families.js', './js/domain/srs/constants.js', './js/domain/srs/scheduler.js', './js/utils/helpers.js', './content/manifest.js', ...UNIT_URLS]) {
    let response;
